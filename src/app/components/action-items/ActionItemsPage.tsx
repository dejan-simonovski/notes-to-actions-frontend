    ...
    <div className="p-4 sm:p-6 md:p-8 h-full flex flex-col min-h-0">
      <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col min-h-0">
        <div className="mb-6 sm:mb-8 flex-shrink-0">
          <h1 className="text-gray-900 mb-1 sm:mb-2">
            Action Items Board
          </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 flex-1 min-h-0">
          {columns.map((column) => {
            ...
            return (
              <div
                key={column.id}
                className="bg-gray-50 rounded-xl p-4 flex flex-col border-2 border-gray-200 overflow-hidden min-h-0"
              >
                ...
                <div className="flex-1 overflow-y-auto min-h-0 space-y-3">
                  {items.map((item) => (
                    <div
                      key={item.id}
                      className="bg-white p-3 sm:p-4 rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow cursor-pointer"
                    >
