...
import { clearMeetings } from "../../store/meetingsSlice";
import type { AppDispatch } from "../../store/store";

export function Dashboard() {
  const dispatch = useDispatch<AppDispatch>();
  const { meetings, totalMeetings, openActionItems, completedTasks } = useDashboard();
      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6 sm:mb-8">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={() => dispatch(clearMeetings())}
              className="flex items-center justify-center gap-2 border border-red-300 text-red-600 px-4 py-2.5 rounded-lg hover:bg-red-50"
            >
              <Trash2 size={18} />
              Clear All
            </button>
          ...
          <div className="divide-y">
            {meetings.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-center">
                <Calendar className="text-gray-300 mb-4" size={40} />
